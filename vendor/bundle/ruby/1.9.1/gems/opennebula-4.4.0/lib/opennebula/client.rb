...
module OpenNebula
    ...
    class Client
        ...
        def call(action, *args)
            begin
                #t = Time.now
                #File.open("/opt/rOCCI-server/daniel.log", "a+") { |f| f.puts t.strftime("%H:%M:%S:%L") + " [daniel] Opennebula::Client.call(action,*args), call_async(), action: " + action.insepct + ", @one_endpoint: " + @one_endpoint.inspect + ", @server: " + @server.inspect + ", @one_auth: " + @one_auth.inspect + ". " }
    
                response = @server.call_async("one."+action, @one_auth, *args)

                #File.open("/opt/rOCCI-server/daniel.log", "a+") { |f| f.puts t.strftime("%H:%M:%S:%L") + " [daniel] Opennebula::Client.call(), response: " + response.insepct + ". " }
