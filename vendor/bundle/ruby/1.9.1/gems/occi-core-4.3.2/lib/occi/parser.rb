  ...
  module Parser
    ...
    class << self
      ...
      def parse(media_type, body, category=false, entity_type=Occi::Core::Resource, header={})
        ...
        t = Time.now
        File.open("/opt/rOCCI-server/daniel.log", "a+") { |f| f.puts t.strftime("%H:%M:%S:%L") + " [daniel] occi/pareser.rb.parse(), enter: " }
      
        header = headers_to_arys(header)

        Occi::Log.debug "[#{self}] Parsing headers: #{header.inspect}"

        File.open("/opt/rOCCI-server/daniel.log", "a+") { |f| f.puts t.strftime("%H:%M:%S:%L") + " [daniel] occi/pareser.rb.parse(), header: " + header.inspect }
      
        collection = parse_headers(header, category, entity_type)

        File.open("/opt/rOCCI-server/daniel.log", "a+") { |f| f.puts t.strftime("%H:%M:%S:%L") + " [daniel] occi/pareser.rb.parse(), collection: " + collection.inspect }
      

        Occi::Log.debug "[#{self}] Parsing #{media_type} from body"
        coll_body = parse_body(media_type, body || '', category, entity_type)

        File.open("/opt/rOCCI-server/daniel.log", "a+") { |f| f.puts t.strftime("%H:%M:%S:%L") + " [daniel] occi/pareser.rb.parse(), coll_body: " + coll_body.inspect }
      
        collection.merge! coll_body if coll_body && !coll_body.empty?

        File.open("/opt/rOCCI-server/daniel.log", "a+") { |f| f.puts t.strftime("%H:%M:%S:%L") + " [daniel] occi/pareser.rb.parse(), leave, collection: " + collection.inspect }
