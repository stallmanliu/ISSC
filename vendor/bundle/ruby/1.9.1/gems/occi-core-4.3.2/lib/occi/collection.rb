...
module Occi
  class Collection
    ...
    def initialize(collection={}, model = Occi::Model.new)
      puts "daniel: Collection.initialize(), input collection:" + collection.inspect
      collection = Hashie::Mash.new(collection) unless collection.kind_of? Occi::Collection
      puts "daniel: Collection.initialize(), get new collection:" + collection.inspect
      ...
      puts "daniel: Collection.initialize(), go to merge collection.mixins.to_a.collect:" + collection.mixins.to_a.collect.inspect
      ...
      puts "daniel: Collection.initialize(), after merge @mixins:" + @mixins.inspect
      
      puts "daniel: Collection.initialize(), go to merge collection.actions.to_a.collect:" + collection.actions.to_a.collect.inspect
      ...
      puts "daniel: Collection.initialize(), after merge collection.actions, @actions:" + @actions.inspect
      ...
      puts "daniel: Collection.initialize(), go to @action = ?, collection.action:" + collection.action.inspect
      @action = Occi::Core::ActionInstance.new(collection.action.action, collection.action.attributes) if collection.action
      puts "daniel: Collection.initialize(), after @action = ?, @action:" + @action.inspect

      puts "daniel: Collection.initialize(), new collection:" + collection.inspect + ", @mixins:" + @mixins.inspect + ", @actions:" + @actions.inspect
    ...
    def check(incl_categories = false, set_default_attrs = false)
      t = Time.now
      File.open("/opt/rOCCI-server/daniel.log", "a+") { |f| f.puts t.strftime("%H:%M:%S:%L") + " [daniel] occi/collection.rb.check(), enter, @resources:" +@resources.inspect + ", @links:" + @link.inspect + ", @action:" + @action.inspect }
      ...
      File.open("/opt/rOCCI-server/daniel.log", "a+") { |f| f.puts t.strftime("%H:%M:%S:%L") + " [daniel] occi/collection.rb.check(), go to check, @@kinds:" +@kinds.inspect + ", @@mixins:" + @mixins.inspect + ", @action:" + @action.inspect }
      ...
      File.open("/opt/rOCCI-server/daniel.log", "a+") { |f| f.puts t.strftime("%H:%M:%S:%L") + " [daniel] occi/collection.rb.check(), go to leave." }
    ...
    def get_by_id(id, cats_only = false)
      ...
      puts "daniel: collection.get_by_id(): id:" + id.inspect + ", self.categories:" + self.categories.inspect + ", self.entities:" + self.entities.inspect
