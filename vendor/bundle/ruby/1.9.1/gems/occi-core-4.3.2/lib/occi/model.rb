module Occi
  class Model < Occi::Collection
    ...
    def register_infrastructure
      ...
      puts "daniel: register_infrastructure(): Occi::Infrastructure.categories:" + Occi::Infrastructure.categories.inspect
    ...
    def register(category)
      ...
      category.model = self
      #puts "daniel: Occi::Model.#: self:" + self.inspect
      #puts "daniel: Occi::Model.register(category): category:" + category.inspect + ", category.class.ancestors:" + category.class.ancestors.inspect
      #puts "daniel: Occi::Model.register(category): @kind:" + @kind.inspect
      #puts "daniel: Occi::Model.register(category): @actions:" + @actions.inspect
      #puts "daniel: Occi::Model.register(category): @mixins:" + @mixins.inspect
      File.open("/opt/rOCCI-server/categories.log", "a+") { |f| f.puts " " }
      File.open("/opt/rOCCI-server/categories.log", "a+") { |f| f.puts "{" }
      File.open("/opt/rOCCI-server/categories.log", "a+") { |f| f.puts "\"" + category.class.name + "\": " + category.inspect }
      File.open("/opt/rOCCI-server/categories.log", "a+") { |f| f.puts "}," }
